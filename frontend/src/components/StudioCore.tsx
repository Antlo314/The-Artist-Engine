...
export default function StudioCore() {
    ...
    const Knob = ({ label, value, onChange }: any) => (
        <div className="flex flex-col items-center gap-3">
            <div className="relative w-16 h-16 rounded-full bg-black border-2 border-blue-900 shadow-[inset_0_0_10px_rgba(0,0,0,0.8)] flex items-center justify-center group">
                <div
                    className="w-1 h-3 bg-blue-400 absolute top-1 origin-[50%_28px] rounded-full shadow-[0_0_5px_rgba(16,185,129,0.8)] transition-transform duration-300 group-hover:bg-white"
                    style={{ transform: `rotate(${(value / 100) * 270 - 135}deg)` }}
                />
                <div className="text-blue-900 font-mono text-xs">{value}%</div>
            </div>
    ...
    const WaveformViewer = ({ url, title, color, renderHeatmap }: { url: string | null, title: string, color: string, renderHeatmap?: boolean }) => {
        ...
        return (
            <div className="flex flex-col gap-2 bg-white/40 shadow-sm p-4 rounded border border-blue-900/10 relative">
                <div className="flex justify-between items-center z-10">
                    ...
                    >
                        <Play size={10} className="text-blue-900 ml-0.5" />
                    </button>
                </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-end justify-between border-b border-blue-900/10 pb-4">
                <div>
                    <h2 className="font-cinzel text-3xl font-bold text-blue-900 tracking-widest text-glow flex items-center gap-3">
                        <Mic2 className="text-blue-700" />
                        AUDIO MASTER CORE
                    </h2>
                    <p className="font-mono text-xs text-gray-600 tracking-widest uppercase mt-1 text-blue-600 drop-shadow-md">
                        Neural Processing Engine // Matchering Framework
                    </p>
                </div>
                <div className="hidden md:flex flex-col items-end">
                    <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-mono text-blue-700 border border-blue-500/30 px-2 rounded">
                            LUFS OMEGA
                        </span>
                    </div>
            ...
            <AnimatePresence mode="wait">
                {phase === 'dropzone' && (
                    ...
                    >
                        ...
                        <div
                            ...
                            onDrop={handleDrop}
                            className={`md:col-span-2 glass-card rounded-2xl p-10 flex flex-col items-center justify-center border-dashed border-2 hover:border-blue-400/50 transition-all min-h-[400px] relative overflow-hidden group ${isDragging ? 'border-blue-400 bg-blue-100/50 shadow-[0_0_30px_rgba(16,185,129,0.2)]' : 'border-blue-200 hover:bg-blue-50/50 cursor-default'}`}
                        >
                            ...
                            <div className="w-20 h-20 rounded-full border-2 border-dashed border-blue-300 flex items-center justify-center mb-6 group-hover:rotate-180 transition-transform duration-1000 ease-in-out">
                                <UploadCloud size={32} className="text-blue-700" />
                            </div>
                            <h3 className="font-cinzel text-2xl text-blue-900 mb-2 tracking-wider">INITIATE INGEST SEQUENCE</h3>
                            ...
                            <div className="flex flex-col gap-4 w-full px-8 z-10">
                                ...
                                <div
                                    ...
                                    className={`flex items-center justify-between w-full py-3 px-4 rounded border transition-all cursor-pointer ${targetFile ? 'bg-blue-100 border-blue-400 shadow-[inset_0_0_10px_rgba(16,185,129,0.2)]' : 'border-blue-900/10 hover:border-white/30 hover:bg-white/5'}`}
                                >
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <div className={`w-8 h-8 rounded flex items-center justify-center shrink-0 ${targetFile ? 'bg-blue-100 text-blue-700' : 'bg-white/5 text-gray-600'}`}>
                                            <Mic2 size={14} />
                                        </div>
                                        <div className="flex flex-col truncate">
                                            <span className={`font-mono text-[10px] tracking-widest uppercase ${targetFile ? 'text-blue-800' : 'text-gray-600'}`}>Target Payload</span>
                                            <span className={`font-mono text-xs truncate ${targetFile ? 'text-blue-900' : 'text-gray-600'}`}>
                                    ...
                                    {targetFile && (
                                        <div className="flex items-center gap-2 shrink-0">
                                            <button
                                                ...
                                                className="w-8 h-8 rounded-full border border-blue-300 flex items-center justify-center text-blue-700 hover:bg-blue-100 hover:scale-105 transition-all"
                                ...
                                <div
                                    ...
                                    className={`flex items-center justify-between w-full py-3 px-4 rounded border transition-all cursor-pointer ${refFile ? 'bg-blue-100 border-blue-400 shadow-[inset_0_0_10px_rgba(16,185,129,0.2)]' : 'border-blue-900/10 hover:border-white/30 hover:bg-white/5'}`}
                                >
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <div className={`w-8 h-8 rounded flex items-center justify-center shrink-0 ${refFile ? 'bg-blue-100 text-blue-700' : 'bg-white/5 text-gray-600'}`}>
                                            <Volume2 size={14} />
                                        </div>
                                        <div className="flex flex-col truncate">
                                            <span className={`font-mono text-[10px] tracking-widest uppercase ${refFile ? 'text-blue-800' : 'text-gray-600'}`}>Acoustic Reference</span>
                                            <span className={`font-mono text-xs truncate ${refFile ? 'text-blue-900' : 'text-gray-600'}`}>
                            ...
                            <div className="mt-6 flex flex-col items-center gap-2 relative z-10 w-full">
                                <label className="font-mono text-[10px] text-gray-600 tracking-widest uppercase">Export Format</label>
                                <div className="flex gap-2">
                                    {['wav', 'mp3', 'flac'].map(fmt => (
                                        <button
                                            key={fmt}
                                            onClick={() => setOutputFormat(fmt)}
                                            className={`px-3 py-1 text-xs font-mono uppercase tracking-widest rounded border transition-colors ${outputFormat === fmt ? 'bg-white/10 border-white text-blue-900' : 'border-blue-900/10 text-gray-600 hover:border-white/30'}`}
                                        >
                                            .{fmt}
                                        </button>
                                    ))}
                        ...
                        <div className="md:col-span-1 glass-card rounded-2xl p-6 flex flex-col gap-4 relative overflow-hidden">
                            <h3 className="font-mono text-[11px] tracking-widest text-blue-700 border-b border-blue-200 pb-2 flex items-center justify-between">
                                <span className="flex items-center gap-2"><Activity size={14} /> AUDIO INSIGHTS</span>
                                {isOracleScanning && <span className="animate-pulse bg-blue-50/500 text-blue-600 px-2 rounded">SCANNING OMEGA CORE...</span>}
                            </h3>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-4">
                                ...
                                {targetFile && !oracleData && !isOracleScanning && (
                                    <button
                                        ...
                                        className="w-full py-4 mt-2 bg-blue-100/50 text-blue-700 border border-blue-500/30 rounded font-mono text-xs tracking-widest hover:bg-blue-800/40 transition-colors shadow-[0_0_15px_rgba(16,185,129,0.1)] shrink-0"
                                    >
                                        [ DEPLOY ORACLE SCAN ]
                                    </button>
                                )}
                                ...
                                {oracleData && !isOracleApplied && (
                                    ...
                                    >
                                        <div className="bg-white/60 shadow-sm p-4 rounded border border-blue-900/10 max-h-[140px] overflow-y-auto">
                                            <p className="font-mono text-[11px] text-blue-100/80 leading-relaxed tracking-wider">
                                                {oracleData.analysis}
                                            </p>
                                        </div>
                                        <div className="bg-white/40 shadow-sm p-2 flex justify-between rounded border border-blue-900/10 font-mono text-[10px] text-gray-700">
                                        ...
                                        <button
                                            ...
                                            className="w-full py-3 bg-blue-500 text-black font-bold border border-blue-400 rounded font-mono text-xs tracking-widest hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] shrink-0"
                                        >
                                            [ APPLY SETTINGS ]
                                        </button>
                                    </motion.div>
                                ...
                                {isOracleApplied && (
                                    <motion.div
                                        ...
                                        className="flex flex-col items-center justify-center gap-4 mt-2 border border-blue-500/30 bg-blue-100/50 rounded p-6 shadow-[inset_0_0_20px_rgba(16,185,129,0.1)] shrink-0"
                                    >
                                        <Shield size={32} className="text-blue-700 drop-shadow-md" />
                                        <div className="flex flex-col items-center">
                                            <h4 className="font-cinzel text-blue-700 font-bold tracking-widest mb-1">TARGET ACQUIRED</h4>
                                            <p className="font-mono text-[10px] text-blue-600 uppercase tracking-widest text-center">
                                ...
                                {targetFile && (
                                    <div className="mt-auto border-t border-blue-200 pt-4 pb-2">
                                        <h3 className="font-mono text-[11px] tracking-widest text-blue-700 mb-4 flex items-center gap-2">
                                        ...
                                        {!isExtractingStems && !stemsData && (
                                            <button
                                                ...
                                                className="w-full py-3 bg-white/5 text-gray-300 border border-blue-900/10 rounded font-mono text-[10px] tracking-widest hover:bg-white/10 hover:border-blue-300 hover:text-blue-700 transition-colors shrink-0"
                                            >
                                            ...
                                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-2 gap-2">
                                                {Object.entries(stemsData).map(([key, url]) => (
                                                    <a
                                                        ...
                                                        className="flex items-center gap-2 justify-center py-2 bg-blue-900/30 text-blue-600 font-mono text-[9px] uppercase tracking-widest border border-blue-500/20 rounded hover:bg-blue-800/50 hover:border-blue-400 transition-colors"
                                                    >
                ...
                {phase === 'processing' && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="glass-card rounded-2xl h-[400px] flex flex-col items-center justify-center border-blue-500 bg-blue-50/50 relative overflow-hidden"
                    >
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                            <div className="relative flex items-center justify-center p-10">
                                <div className="absolute inset-0 border-t-2 border-blue-400 rounded-full animate-spin [animation-duration:3s]" />
                                <div className="absolute inset-4 border-r-2 border-blue-200 rounded-full animate-spin [animation-duration:2s] [animation-direction:reverse]" />
                                <Mic2 size={48} className="text-blue-700 animate-pulse" />
                            </div>
                    ...
                    >
                        ...
                        <div className="glass-card p-6 flex items-center justify-between rounded-2xl border-blue-900 shadow-[0_0_30px_rgba(16,185,129,0.1)]">
                            <div className="flex items-center gap-6">
                                ...
                                <div className={`transition-all duration-500 ${isPlaying ? 'ml-16' : 'ml-0'}`}>
                                    <h3 className="font-cinzel text-2xl text-blue-900 font-bold tracking-widest mb-1">FINAL_RENDER_AE94.wav</h3>
                                    <div className="flex items-center gap-4 font-mono text-xs text-blue-700 uppercase tracking-widest drop-shadow-md">
                            ...
                            <div className="flex items-center gap-4 bg-white/60 shadow-sm p-2 pr-6 rounded-full border border-blue-900/10">
                                <button
                                    ...
                                    className={`w-12 h-6 rounded-full p-1 transition-colors ${isSovereignMaster ? 'bg-blue-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'bg-gray-700'}`}
                                >
                                    <motion.div
                                ...
                                <div className="flex flex-col">
                                    <span className="font-cinzel text-xs text-blue-900 font-bold -mb-1">SOVEREIGN MODE</span>
                                    <span className="font-mono text-[9px] text-blue-700 tracking-widest uppercase">A/B Testing Active</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="glass-card p-8 rounded-2xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-8 opacity-5">
                                <Settings2 size={200} className="text-blue-700 mix-blend-screen" />
                            </div>

                            <h3 className="font-cinzel text-xl text-blue-900 tracking-widest mb-8 border-b border-blue-900/10 pb-4 inline-block pr-12">
                                FINE-TUNING CONSOLE
                            </h3>
                                ...
                                {masterAudioUrl && (
                                    <>
                                        <button
                                            ...
                                            className="font-mono text-xs text-blue-700 hover:text-blue-900 border-b border-blue-600 pb-1 tracking-widest uppercase transition-colors flex items-center gap-2"
                                        >
                ...
                {showAnalytics && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white/90 border border-blue-900/10 shadow-sm w-full max-w-4xl border border-blue-500/30 shadow-[0_0_50px_rgba(16,185,129,0.15)] rounded-2xl overflow-hidden flex flex-col max-h-[90vh]"
                        >
                            <div className="p-4 border-b border-blue-900/10 flex justify-between items-center bg-white/40 shadow-sm">
                                <h2 className="font-cinzel text-xl font-bold text-blue-900 tracking-widest flex items-center gap-3">
                                ...
                                <div className="flex items-center gap-4">
                                    <button
                                        ...
                                        className={`font-mono text-[9px] uppercase tracking-widest py-1 px-3 border rounded transition-colors ${showHeatmap ? 'bg-blue-100 text-blue-700 border-blue-300' : 'bg-transparent text-gray-600 border-gray-600'}`}
                                    >
                            ...
                            <div className="p-6 overflow-y-auto custom-scrollbar flex flex-col gap-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="flex flex-col h-full bg-white/40 shadow-sm border border-blue-900/10 rounded p-6">
                                        <h3 className="font-mono text-xs text-blue-700 tracking-widest uppercase mb-4 flex items-center gap-2">
                                        ...
                                        <div className="text-xs font-mono text-gray-300 space-y-3 flex-1">
                                            <p className="border-l-2 border-blue-500 pl-2">
                                                <span className="text-blue-700 font-bold block mb-1">Phase 1: LUFS & RMS Alignment</span>
                                            ...
                                            <p className="border-l-2 border-blue-500 pl-2">
                                                <span className="text-blue-700 font-bold block mb-1">Phase 2: Sovereign Clarity Polish</span>
                                ...
                                <div className="mt-4 border border-blue-300 rounded-lg p-1 bg-blue-100/50">
                            ...
                            <div className="p-8 flex flex-col gap-8 relative z-10">
                                ...
                                <div className="grid grid-cols-2 gap-8">
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        >
                                            {blindTestPlaying === 'A' ? <Pause size={32} className="text-orange-400" /> : <Play size={32} className="text-gray-700 group-hover:text-orange-400 ml-2" />}
                                        </button>

                                        {!blindTestWinner ? (
                                            <button
                                                onClick={() => handleBlindTestVote('A')}
                                                className="py-3 bg-white/5 border border-blue-900/10 text-blue-900 font-mono text-xs tracking-widest uppercase hover:bg-orange-600 hover:border-orange-400 transition-colors rounded shadow-md"
                                            >
                                                [ VOTE SOURCE A ]
                                            </button>
                                        ) : (
                                            <div className={`p-4 rounded border font-mono tracking-widest text-center text-xs uppercase ${isSovereignA ? 'bg-blue-900/30 border-blue-500 text-blue-700' : 'bg-red-900/30 border-red-500 text-red-500'}`}>
                                                {isSovereignA ? "🏆 SOVEREIGN MASTER" : "❌ ORIGINAL UNMASTERED"}
                                                {blindTestWinner === 'A' && <div className="mt-2 text-[9px] text-blue-900">YOUR SELECTION</div>}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        >
                                            {blindTestPlaying === 'B' ? <Pause size={32} className="text-orange-400" /> : <Play size={32} className="text-gray-700 group-hover:text-orange-400 ml-2" />}
                                        </button>
                                        ...
                                        {!blindTestWinner ? (
                                            ...
                                            <div className={`p-4 rounded border font-mono tracking-widest text-center text-xs uppercase ${!isSovereignA ? 'bg-blue-900/30 border-blue-500 text-blue-700' : 'bg-red-900/30 border-red-500 text-red-500'}`}>
                                                {!isSovereignA ? "🏆 SOVEREIGN MASTER" : "❌ ORIGINAL UNMASTERED"}
                                                {blindTestWinner === 'B' && <div className="mt-2 text-[9px] text-blue-900">YOUR SELECTION</div>}
                                            </div>
                                        )}
                                    </div>
                                </div>
