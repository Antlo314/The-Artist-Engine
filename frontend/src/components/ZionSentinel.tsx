...
export default function ZionSentinel() {
    ...
    const ScoreGauge = ({ score }: { score: number }) => {
        ...
        return (
            <div className="relative flex flex-col items-center justify-center p-6 border border-purple-900/40 rounded-2xl bg-black/40 backdrop-blur-md shadow-[inset_0_0_20px_rgba(168,85,247,0.1)]">
                    ...
                    if (i % 2 === 0) {
                        ...
                        newElements.push(
                            <details key={`${entry.term}-${partIdx}-${i}`} className={`inline-block group cursor-pointer border-b border-dashed ${c.border} ${c.text} font-bold ${c.bg} px-1 rounded mx-0.5 relative align-bottom`}>
                                ...
                                <div className={`absolute top-full left-0 mt-2 w-64 p-3 bg-black/90 backdrop-blur-md border ${c.panelBorder} rounded-lg shadow-[0_0_30px_rgba(168,85,247,0.3)] z-[100] flex flex-col gap-1 text-left font-normal cursor-default`}>
                                    ...
                                    <span className="font-inter text-xs text-gray-300 leading-tight">"{entry.translation}"</span>
                                    <span className="font-mono text-[9px] text-gray-500 leading-tight mt-1 bg-purple-900/20 p-1 rounded border border-purple-900/30">{entry.meaning}</span>
                                </div>
                            </details>
                        );
                    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {/* Document Ingest Box */}
                <div className={`glass-card rounded-2xl flex flex-col border transition-colors shadow-xl relative overflow-hidden group focus-within:border-purple-900/50 ${isDragging ? 'border-purple-800 bg-purple-100/30' : 'border-purple-900/10'}`}>
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-800 to-transparent" />

                    {/* Toggle Switch */}
                    <div className="p-4 border-b border-purple-900/40 bg-black/40 flex justify-between items-center gap-4">
                    ...
                    {analysis && (
                        ...
                        >
                                ...
                                <div className="glass-card rounded-2xl border border-red-500/30 overflow-hidden relative shadow-[0_0_20px_rgba(239,68,68,0.1)] backdrop-blur-md bg-black/40">
                                    ...
                                    <div className="p-4 space-y-4 bg-black/60">
                                        {analysis.red_flags.map((flag: any, idx: number) => (
                                            <div key={idx} className="bg-black/80 border border-red-500/30 rounded p-4 relative group shadow-sm">
                            ...
                            <div className="glass-card p-6 rounded-2xl border border-purple-900/30 bg-black/40 backdrop-blur-md mt-4 relative shadow-[inset_0_0_20px_rgba(168,85,247,0.1)]">
                                ...
                                <div className="text-sm font-inter text-gray-300 leading-relaxed whitespace-pre-wrap bg-black/60 p-4 rounded border border-purple-900/30 shadow-inner">
